...
<div>
    ...
    <div class="flex flex-wrap gap-4 mb-6 mt-2">
        <div class="flex-1 min-w-[180px] bg-white rounded-lg shadow border border-slate-200 p-4 flex flex-col items-center">
            ...
            <span class="text-2xl font-bold text-slate-600 mt-1">{{ $minhasRequisicoes->where('devolvido_em', null)->count() }}</span>
        </div>
        <div class="flex-1 min-w-[180px] bg-white rounded-lg shadow border border-slate-200 p-4 flex flex-col items-center">
            <span class="block text-[13px] text-blue-900/60 font-semibold uppercase tracking-wider text-center mb-1">Últimos 30 dias</span>
            <span class="text-2xl font-bold text-slate-600 mt-1">{{ $minhasRequisicoes->where('requisitado_em', '>=', now()->subDays(30))->count() }}</span>
        </div>
        <div class="flex-1 min-w-[180px] bg-white rounded-lg shadow border border-slate-200 p-4 flex flex-col items-center">
            ...
            <span class="text-2xl font-bold text-slate-600 mt-1">
                {{ $minhasRequisicoes->filter(function($req) {
                    if (!$req->devolvido_em) return false;
                    try {
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-xs text-slate-500">Nenhuma requisição encontrada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($isAdmin)
        <div class="pt-6 text-left text-sm font-semibold uppercase tracking-wider text-slate-500">Todas as requisições</div>
        ...
        <div class="mb-2">
            <div class="flex flex-wrap gap-6 items-end pt-4 bg-slate-50 rounded-lg px-4 py-3 border border-slate-200">
                ...
                <div class="flex flex-col">
                    <label class="block text-xs font-semibold mb-1 uppercase tracking-wider text-slate-700">Requisitado em</label>
                    <div class="flex gap-2">
                        <input type="date" wire:model.live="adminDataRequisicaoInicio" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="De" />
                        <span class="text-xs text-slate-400 self-center">a</span>
                        <input type="date" wire:model.live="adminDataRequisicaoFim" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="Até" />
                    </div>
                </div>
                <div class="flex flex-col">
                    <label class="block text-xs font-semibold mb-1 uppercase tracking-wider text-slate-700">Devolução prevista</label>
                    <div class="flex gap-2">
                        <input type="date" wire:model.live="adminDataPrevistaInicio" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="De" />
                        <span class="text-xs text-slate-400 self-center">a</span>
                        <input type="date" wire:model.live="adminDataPrevistaFim" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="Até" />
                    </div>
                </div>
                <div class="flex flex-col">
                    <label class="block text-xs font-semibold mb-1 uppercase tracking-wider text-slate-700">Devolvido em</label>
                    <div class="flex gap-2">
                        <input type="date" wire:model.live="adminDataDevolucaoInicio" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="De" />
                        <span class="text-xs text-slate-400 self-center">a</span>
                        <input type="date" wire:model.live="adminDataDevolucaoFim" class="rounded-lg border border-slate-300 px-2 py-2 text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition w-[130px] text-black" placeholder="Até" />
                    </div>
                </div>
            </div>
            <div class="mt-4">
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap align-middle min-w-[180px]">
                                    @if($requisicao->user)
                                        <div class="flex items-center gap-3">
                                            <img src="{{ $requisicao->user->profile_photo_url }}" alt="Foto" class="h-9 w-9 rounded-full object-cover border border-slate-200 shadow-sm bg-white" />
                                            <div class="flex flex-col">
                                                <span class="font-semibold text-slate-900 text-xs">{{ $requisicao->user->name }}</span>
                                                <span class="text-xs text-slate-500">{{ $requisicao->user->email }}</span>
                                            </div>
                                        </div>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-xs text-slate-500">Nenhum histórico de requisições encontrado.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
