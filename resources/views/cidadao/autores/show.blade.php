    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex items-center gap-2 text-sm">
                ...
                <a href="{{ route('autores.index') }}" class="text-xs text-slate-500 underline underline-offset-2">Autores</a>
                <span class="text-xs text-slate-400">/</span>
                <span class="text-xs text-slate-500">{{ $autor->nome }}</span>
            </div>
            ...
            <div class="mb-10 flex flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:p-6">
                ...
                <div>
                    <h1 class="text-3xl font-bold tracking-tight text-slate-900">{{ $autor->nome }}</h1>
                    <p class="mt-2 text-sm text-slate-600">{{ $autor->livros->count() }} {{ $autor->livros->count() === 1 ? 'livro associado' : 'livros associados' }}</p>
                </div>
            </div>
            
            <div class="border-t border-slate-200">
                <h2 class="pt-10 pb-8 text-2xl font-bold text-slate-900">Catálogo do Autor</h2>
            </div>
                ...
                <div class="grid grid-cols-2 gap-4 pb-2 sm:grid-cols-4 lg:grid-cols-6">
                    @foreach ($autor->livros as $livro)
                        @php
                            ...
                            $autorPrincipal = $livro->autores->first()->nome ?? $autor->nome;
            ...
            @else
                <div class="rounded-xl border border-slate-200 bg-white px-4 py-8 text-center text-sm text-slate-500 shadow-sm">
                    Este autor ainda não tem livros associados.
                </div>
            @endif
        </div>
    </div>
